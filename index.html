...
<body class="flex flex-col min-h-screen">
        ...
        <div id="home-screen" class="screen active">
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button class="btn-primary py-3" onclick="openModal('fab-menu-modal')">Add Food</button>
        ...
        <div id="shopping-list-screen" class="screen">
            ...
            <div class="card mb-6">
                ...
                <div class="flex space-x-2">
                    <input type="text" id="manual-shopping-item" placeholder="e.g., Milk, 1 gallon" class="flex-grow">
                    <button class="btn-primary" onclick="addManualShoppingItem()"><i class="fas fa-plus"></i> Add</button>
                </div>
            </div>

            <div class="card mb-6">
                <h2 class="text-xl font-semibold mb-3">Auto-add Missing Ingredients</h2>
                <p class="text-sm text-gray-600 mb-4">Automatically add ingredients you're missing for sample recipes.</p>
                <button class="btn-primary w-full py-3" onclick="addMissingIngredientsToShoppingList()">
                    <i class="fas fa-magic mr-2"></i> Add Missing from Recipes
                </button>
            </div>
    ...
    <script>
        ...
        let showOnlyFavoriteRecipes = false;
        ...
        // Sample Inventory and Recipes for auto-add feature (hardcoded for this standalone app)
        const sampleInventoryForShoppingList = [
            { id: 'inv1', name: 'Milk', quantity: 1, unit: 'gal', expirationDate: '2025-06-05' },
            { id: 'inv2', name: 'Eggs', quantity: 12, unit: 'pcs', expirationDate: '2025-06-10' },
            { id: 'inv3', name: 'Chicken Breast', quantity: 2, unit: 'lb', expirationDate: '2025-06-03' },
            { id: 'inv4', name: 'Flour', quantity: 5, unit: 'lb', expirationDate: '2026-12-31' },
            { id: 'inv5', name: 'Sugar', quantity: 2, unit: 'lb', expirationDate: '2027-01-15' },
            { id: 'inv6', name: 'Tomatoes', quantity: 3, unit: 'pcs', expirationDate: '2025-06-01' }
        ];

        const sampleRecipesForShoppingList = [
            {
                id: 'rec1',
                name: 'Chicken Curry',
                ingredients: [
            ...
            },
            {
                ...
                ingredients: [
                    ...
                    { name: 'Maple Syrup', quantity: '1', unit: 'bottle' }
                ]
            },
            {
                id: 'rec3',
        ...
        function handleOutcome() {
            ...
            if (itemIndex > -1) {
                ...
                if (outcomeType === 'ate') {
                    ...
                    // Ask to add to shopping list when consumed
                    if (confirm(`"${item.name}" marked as consumed. Do you want to add it to your shopping list for next time?`)) {
        ...
        function addGestureListeners(element) {
            ...
            itemCard.addEventListener('touchstart', (e) => {
                ...
                longPressTimer = setTimeout(() => {
                    isLongPress = true;
                    openModal('update-portion-modal', itemCard.dataset.itemId); // Use openModal
            ...
            itemCard.addEventListener('touchend', (e) => {
                ...
                if (isSwiping) {
                    if (diffX < -SWIPE_THRESHOLD) { // Swiped left (Expired)
                        ...
                        if (item && confirm(`"${item.name}" marked as consumed. Do you want to add it to your shopping list for next time?`)) {
                            shoppingListItems.push({
                                id: `shop${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
                                name: item.name,
                                quantity: item.quantity,
        ...
        function addTakeoutToInventory() {
            ...
            if (!dishName || !cuisineType || !storageLocation || !dateAdded) {
                alert('Please fill in all required fields for takeout entry.');
                return;
            }
            ...
            alert(`${dishName} (Takeout) added to inventory!`);
        }
        ...
        async function processReceiptImage() {
            ...
            try {
                ...
                alert('Error processing receipt: ' + error.message + '. Ensure the Flask backend is running and accessible (if using one).');
        ...
        function renderRecipes(recipesToRender) {
            ...
            let recipesToDisplay = showOnlyFavoriteRecipes ? recipesToRender.filter(recipe => favoriteRecipes.includes(recipe.id)) : recipesToRender;
            ...
            if (recipesToDisplay.length === 0) {
                ...
                if (showOnlyFavoriteRecipes) {
                    recipeListDiv.innerHTML = '<p class="text-center text-gray-500 mt-8 col-span-full">No favorited recipes.</p>';
                }
                return;
            }
        ...
        function toggleFavoriteItem(itemId) {
            if (favoriteInventoryItems.includes(itemId)) {
                favoriteInventoryItems = favoriteInventoryItems.filter(id => id !== itemId);
                alert('Item removed from inventory favorites!');
            } else {
                favoriteInventoryItems.push(itemId);
                alert('Item added to inventory favorites!');
            }
        ...
        function toggleFavorite(recipeId) {
            ...
            saveFavoritesToLocalStorage();
            applyRecipeFilters(); // Re-render recipe list to update heart icons
        ...
        function renderFavoriteRecipes() {
            ...
            if (favRecipesToRender.length === 0 && favoriteRecipes.length > 0) {
                // This case happens if favoriteRecipes has IDs but the actual recipes are not in generatedRecipes (e.g., after refresh without regenerating)
        ...
        function addManualShoppingItem() {
            ...
            const commonUnits = ['pcs', 'ea', 'bag', 'box', 'can', 'bottle', 'pack', 'bunch', 'head', 'lbs', 'oz', 'g', 'kg', 'fl oz', 'cup', 'pint', 'quart', 'gal', 'ml', 'L', 'tsp', 'tbsp', 'dozen'];
            if (parts.length >= 2) {
                const lastPart = parts[parts.length - 1].toLowerCase();
                const secondLastPart = parts[parts.length - 2];

                if (commonUnits.includes(lastPart)) {
                    unit = lastPart;
                    ...
                    quantity = parseFloat(lastPart);
                    name = parts.slice(0, -1).join(' ');
                }
            }
            if (!name) name = itemText; // Fallback if parsing fails
            ...
            if (itemIndex > -1) {
                ...
                if (confirm(`Are you sure you want to remove "${removedItem.name}" from your shopping list?`)) {
                    ...
                    if (confirm(`"${removedItem.name}" removed. Do you want to add it to your shopping list again for next time?`)) {
                        ...
                        saveShoppingListToLocalStorage();
                        renderShoppingList();
                        alert(`"${removedItem.name}" has been re-added to your list.`);
                    } else {
        ...
        // --- Auto-add Missing Ingredients (for Shopping List) ---
        function checkIngredientAvailabilityForShoppingList(recipeIngredients, currentInventory) {
            let missingIngredients = [];
            ...
            recipeIngredients.forEach(recIng => {
                ...
                if (!foundInInventory) {
                    const alreadyOnList = shoppingListItems.some(shopItem =>
                        shopItem.name.toLowerCase().includes(recIng.name.toLowerCase()) && !shopItem.bought
                    );
                    if (!alreadyOnList) {
                        missingIngredients.push({
                            name: recIng.name,
                            quantity: recIng.quantity || 1,
                            unit: recIng.unit || 'pcs',
                            source: 'From Recipe Suggestion'
                        });
                    }
                }
            });
        ...
        function addMissingIngredientsToShoppingList() {
            showLoadingOverlay('Checking recipes and inventory...');
            let newItemsAddedCount = 0;

            sampleRecipesForShoppingList.forEach(recipe => { // Use sampleRecipesForShoppingList
                const missing = checkIngredientAvailabilityForShoppingList(recipe.ingredients, sampleInventoryForShoppingList); // Use sampleInventoryForShoppingList
                missing.forEach(item => {
                    shoppingListItems.push({
                        id: `autoadd${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
                        name: item.name,
                        quantity: item.quantity,
                        unit: item.unit,
                        source: item.source,
            ...
            if (newItemsAddedCount > 0) {
                alert(`${newItemsAddedCount} missing ingredients added to your shopping list!`);
            } else {
                alert('No new missing ingredients found from sample recipes. Your inventory is well-stocked!');
            }
        }
        ...
        function updateFoodConsumedSummary() {
            ...
            if (filterPeriod === 'Daily') {
                filteredConsumed = consumedItems.filter(item => {
                    ...
                    return consumedDate.getTime() === today.getTime();
                });
                ...
                filteredConsumed = consumedItems.filter(item => {
                    const consumedDate = new Date(item.dateConsumed);
                    consumedDate.setHours(0, 0, 0, 0);
                    return consumedDate >= startOfWeek && consumedDate < endOfWeek;
                });
                ...
                filteredConsumed = consumedItems.filter(item => {
                    const consumedDate = new Date(item.dateConsumed);
                    consumedDate.setHours(0, 0, 0, 0);
                    return consumedDate >= startOfMonth && consumedDate <= endOfMonth;
                });
            }

            let totalConsumedItemsCount = filteredConsumed.length;

            const periodText = filterPeriod.toLowerCase();
            document.getElementById('consumed-amount-display').textContent = `${totalConsumedItemsCount} items consumed this ${periodText}`;
